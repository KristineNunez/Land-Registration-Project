...
class LandRegNFT(FA2.Admin, FA2.Fa2Nft, FA2.BurnNft):
    ...
    def register_land(self, size, tax_value, image, loc):
        ...
        metadata = self.make_metadata(
            size=size, tax_value=tax_value, image=image, loc=loc
        )
        ...
        self.data.encumbrance[token_id] = sp.record(
            type="",
            date=sp.timestamp(0),
            amount=sp.tez(0),
            liability=sp.sender,
    ...
    def add_encumbrance(self, reg_num, type, date, amount, liability, months):
        ...
        sp.verify(
            self.data.encumbrance[reg_num].type == "", "LAND ALREADY HAS ENCUMBRANCE"
        )
    ...
    def remove_encumbrance(self, reg_num):
        ...
        sp.verify(
            ...
            "LAND HAS NO EXISTING ENCUMBRANCE",
        )
    ...
    def transfer_title(self, reg_num, params):
        ...
        sp.verify(
            self.data.encumbrance[reg_num].type == "",
            "CANNOT TRANSFER TITLE IF LAND HAS ENCUMBRANCES",
        )
        ...
        with sp.forBlock("transfer", params) as transfer:
            with sp.forBlock("tx", transfer.txs) as tx:
                sp.verify(
    ...
    def burn_title(self, reg_num):
        params = [sp.record(amount=1, from_=sp.sender, token_id=reg_num)]

        # Burn entrypoint from BurnNft mixin
        sp.set_type(
            params,
            sp.TList(
                sp.TRecord(amount=sp.TNat, from_=sp.TAddress, token_id=sp.TNat).layout(
                    ("from_", ("token_id", "amount"))
                )
            ),
        )
        sp.verify(True, "FA2_TX_DENIED")
        with sp.forBlock("action", params) as action:
            sp.verify(
                self.data.token_metadata.contains(action.token_id),
                "FA2_TOKEN_UNDEFINED",
            )
            sp.verify(
                (sp.sender == action.from_)
                | (
                    self.data.operators.contains(
                        sp.record(
                            operator=sp.sender,
    ...
    def buy_land(self, reg_num):
        ...
        sp.verify(
            self.data.ledger[reg_num] != sp.source, "BUYER IS CURRENT OWNER OF LAND"
        )
        ...
        with sp.if_(extra_amount > sp.tez(0)):
        ...
        self.transfer_title(
            ...
            [
                sp.record(
                    from_=sp.sender,
                    txs=[sp.record(to_=sp.source, amount=sp.nat(1), token_id=reg_num)],
                )
            ],
        )
    ...
    def pay_lease(self, reg_num):
        ...
        # Check if lessee
        sp.verify(
            sp.sender == self.data.encumbrance[reg_num].liability, "NOT THE LESSEE"
        )

        # Check if amount is greater than or equal to pay
        sp.verify(
            sp.amount >= self.data.encumbrance[reg_num].amount, "NOT ENOUGH FOR PAYMENT"
        )
        ...
        self.data.encumbrance[reg_num].months = (
            self.data.encumbrance[reg_num].months - 1
        )

        # If lease period is done, remove encumbrance
        with sp.if_(self.data.encumbrance[reg_num].months == 0):
    ...
    def pay_mortgage(self, reg_num):
        ...
        sp.verify(
            sp.source == self.data.encumbrance[reg_num].liability, "NOT THE LOAN PERSON"
        )
            ...
            self.transfer_title(
                ...
                [
                    sp.record(
                        from_=sp.sender,
                        txs=[
                            sp.record(to_=sp.source, amount=sp.nat(1), token_id=reg_num)
                        ],
                    )
                ],
            )
...
def test():
    ...
    nft = LandRegNFT(
        admin=admin.address, metadata=sp.utils.metadata_of_url("http://example.com")
    )
    ...
    nft.register_land(
        ...
        tax_value="2",
        loc="Pasay City",
        image="ipfs://QmRbmXcd2yfNVdgHL7oYWS2yd3tztr2NZiqP2LFuw3voPW",
    ).run(sender=alice)
    nft.register_land(
        size="17",
        tax_value="8",
    ...
    nft.add_encumbrance(
        reg_num=1, type="lease", date=sp.now, amount=sp.tez(5), months=sp.int(1)
    ).run(sender=bob, source=carol)
    nft.pay_lease(1).run(sender=carol, amount=sp.tez(5))

    nft.add_encumbrance(
        reg_num=0, type="mortgage", date=sp.now, amount=sp.tez(6), months=sp.int(1)
    ).run(sender=bob, source=carol)
